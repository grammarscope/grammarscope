 ...
 */
class DownloadZipCore(progressEmitter: ProgressEmitter<Pair<Long, Long>>) : DownloadCore(progressEmitter) {
    ...
    override suspend fun work(fromUrl: String, toFile: String, renameFrom: String?, renameTo: String?, entry: String?): DownloadData? {
     ...
     */
    @Throws(Exception::class)
    override suspend fun job(): DownloadData {

        return withContext(Dispatchers.IO) {

            // first
            prerequisite()
            val outDir = File(toFile)
            ...
            try {
                ...
                // emitProgress(count.toLong() to max.toLong())
                            ...
                            .use { zis ->
                                generateSequence { zis.nextEntry }
                                    ...
                                    .forEach { it ->
                                        ...
                                        // files
                                        val tempOutFile = File(outDir, "$entryName.part")
                                        val entryOutFile = File(outDir, entryName)
                                        if (!entryOutFile.canonicalPath.startsWith(dirPath)) {
                                        ...
                                        if (entryName == renameFrom && renameTo != null) {
                                            val renamed = renameTo?.run { File(outDir, this) }
            ...
            Log.d(TAG, "Downloaded " + outDir.absolutePath)
    ...
    companion object {
        ...
        private const val TAG = "DownloadZipCore"
    }
}
