 ...
 */
class ContentDownloader : BaseTask<String, Array<String>?>() {
    ...
    companion object {
        ...
        fun addTargetToIntent(context: Context, intent: Intent, name: String): Intent {
        ...
        private fun addTargetToIntentZipDownload(context: Context, intent: Intent, name: String): Intent {
            ...
            intent.putExtra(DOWNLOAD_FROM_ARG, source) // source archive
            intent.putExtra(DOWNLOAD_TO_DIR_ARG, dest) // source archive
         ...
         * @param consumer consumer of activity and result
         */
        fun showContents(activity: AppCompatActivity, consumer: BiConsumer<Context, String>) {
            ...
            activity.lifecycleScope.launch {
                val result = ContentDownloader().run(Dispatchers.IO, sourceFile)
                show(activity, result, targetFile, consumer)
            }
        }
    }
}
