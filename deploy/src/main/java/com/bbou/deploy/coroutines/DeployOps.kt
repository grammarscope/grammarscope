...
import com.bbou.deploy.coroutines.Deploy.digestToString
...
/**
 * Deploy ops
 * Functions have a progress emitter parameter that acts as a sink for operation progress
 */
object DeployOps {

    private const val TAG = "ObservedDeploy"
     ...
     * @param progressEmitter progress emitter
     * @param progressRate progress rate
     * @return true if successful
     */
    suspend fun copyFromFile(srcFile: String, destFile: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): Boolean {
            ...
            FileInputStream(srcFile).use { `is` ->
                FileOutputStream(destFile).use { os ->
                    ...
                    while (`is`.read(buffer).also { readCount = it } != -1) {
                        ...
                        // emit progress
                        if (chunkCount % progressRate == 0) {
                            progressEmitter.emitProgress(byteCount to length)
                        }
     ...
     * @return true if successful
     */
    suspend fun copyFromUri(srcUri: Uri, resolver: ContentResolver, destFile: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): Boolean {
            ...
            resolver.openInputStream(srcUri).use { `is` ->
                FileOutputStream(destFile).use { os ->
                    ...
                    while (`is`!!.read(buffer).also { readCount = it } != -1) {
                        ...
                        // emit progress
                        if (chunkCount % progressRate == 0) {
                            progressEmitter.emitProgress(byteCount to -1)
                        }
     ...
     * @param progressRate progress rate
     * @return true if successful
     * @throws SecurityException if zip path traversal attempt
     */
    @Throws(SecurityException::class)
    suspend fun unzipFromArchiveFile(srcArchive: String, destDir: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): Boolean {
        ...
        try {
            ZipFile(srcArchive).use { zipFile ->
                ...
                while (zipEntries.hasMoreElements()) {
                    ...
                    if (!outFile.canonicalPath.startsWith(dirPath)) {
                        throw SecurityException("Zip traversal attempt")
                    }
                    ...
                    } catch (innerE: IOException) {
                        Log.e(TAG, "While executing from archive", innerE)
                    }
                    if (outFile.exists()) {
        ...
        } catch (outerE: IOException) {
            Log.e(TAG, "While executing from archive", outerE)
        }
        return false
    }
     ...
     */
    @Throws(SecurityException::class)
    suspend fun unzipFromArchiveUri(srcUri: Uri, resolver: ContentResolver, destDir: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): Boolean {
     ...
     *
     * @param srcUrl source archive uri
     * @param destDir destination dir
     * @param progressEmitter progress emitter
     * @param progressRate progress rate
    ...
    suspend fun unzipFromArchiveUrl(srcUrl: URL, destDir: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): Boolean {
     ...
     * @return true if successful
     */
    suspend fun unzipEntryFromArchiveUri(srcUri: Uri, srcEntry: String, resolver: ContentResolver, destFile: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): Boolean {
     ...
     */
    suspend fun md5FromFile(srcFile: String, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): String? {
     ...
     * @return digest if successful
     */
    suspend fun md5FromUri(uri: Uri, resolver: ContentResolver, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): String? {
     ...
     * @return digest if successful
     */
    suspend fun md5FromUrl(url: URL, progressEmitter: ProgressEmitter<Pair<Number, Number>>, progressRate: Int): String? {
