...
object Deploy {
     ...
     */
    @Suppress("unused")
    suspend fun copyFile(fromPath: String, toPath: String, publisher: ProgressEmitter<Pair<Long, Long>>, publishRate: Int): Boolean {
        ...
        try {
            FileInputStream(fromPath).use { `is` ->
                FileOutputStream(toPath).use { os ->
                    copyStreams(`is`, os, length, publisher, publishRate)
                    return true
                }
            }
        } catch (e: Exception) {
     ...
     */
    @Throws(IOException::class)
    private suspend fun copyStreams(`is`: InputStream, os: OutputStream, length: Long, publisher: ProgressEmitter<Pair<Long, Long>>, publishRate: Int) {
        val buffer = ByteArray(CHUNK_SIZE)
        ...
        while (`is`.read(buffer).also { readCount = it } != -1) {
            ...
            if (chunkCount % publishRate == 0) {
                publisher.emitProgress(byteCount to length)
            }
    ...
    private fun expandZipStream(`is`: InputStream, @Suppress("SameParameterValue") pathPrefixFilter: String?, toDir: File, flatten: Boolean): File {
        ...
        toDir.mkdir()
        val dirPath = toDir.canonicalPath
        ...
        ZipInputStream(`is`).use { zis ->
            ...
            generateSequence { zis.nextEntry }
                ...
                .forEach { it ->
                    ...
                    val outFile = if (flatten) File(dirPath + File.separator + File(it.name).name) else File(toDir.toString() + File.separator + it.name)
    ...
    suspend fun unzipFromArchive(fromPath: String, toPath: String, publisher: ProgressEmitter<Pair<Long, Long>>, publishRate: Int): Boolean {
        Log.d(TAG, "Expanding from $fromPath to $toPath")
        val dirPath = File(toPath).canonicalPath
     ...
     */
    @Suppress("unused")
    suspend fun unzipEntryFromArchive(fromPath: String, fromEntry: String, toFilePath: String, publisher: ProgressEmitter<Pair<Long, Long>>, publishRate: Int): Boolean {
     ...
     */
    @Suppress("unused")
    suspend fun md5FromFile(fromPath: String, publisher: ProgressEmitter<Pair<Long, Long>>, publishRate: Int): String? {
    ...
    private fun computeDigest(path: String): String? {
        // Log.d(TAG, "MD5 " + path);
